...
- `StaffService` will query the database to find staff who have at least one skill that partially matches the provided technologies.
- Instead of an exact match (`IN`), the query will use fuzzy matching (e.g., `contains` with `mode: 'insensitive'` in Prisma) for each technology provided. For example, if a user types "react.js", it should match a staff skill named "React" (or vice-versa).
