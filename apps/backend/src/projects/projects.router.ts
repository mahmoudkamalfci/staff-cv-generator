...
projectsRouter.post('/', requireAuth, requireAdmin, validate(CreateProjectSchema), asyncHandler(async (req, res) => {
...
projectsRouter.patch('/:id', requireAuth, requireAdmin, validate(UpdateProjectSchema), asyncHandler(async (req, res) => {
...
projectsRouter.delete('/:id', requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const project = await prisma.project.findUnique({ where: { id: req.params.id } });
  if (!project) throw new AppError(404, 'Project not found');
